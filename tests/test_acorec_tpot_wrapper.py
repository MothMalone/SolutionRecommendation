...
def test_wrapper_uses_train_60_and_scores_outer_test_20_only():
    ...
    # ACORec never drops test rows: the coverage fields let this row join the AutoDP table.
    assert result["test_coverage"] == 1.0
    assert result["score_full"] == result["score"]
    assert result["score_kept"] == result["score"]
