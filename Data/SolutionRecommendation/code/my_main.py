...
def load_test_datasets(test_dataset_ids):
    """Load actual OpenML datasets for evaluation"""
    ...
    for dataset_id in test_dataset_ids:
        try:
            ...
            datasets[dataset_id] = {'X': X, 'y': y}
            print(f"Loaded D_{dataset_id}: {X.shape}")
        except Exception as e:
            print(f"Failed to load D_{dataset_id}: {e}")
...
if __name__=='__main__':
    ...
    if args.is_bayes==1:
        ...
        if args.is_bayes==1:
            # ... existing GPLVM training code stays the same ...
            
            '''Evaluate with AutoGluon'''
            print("\n" + "="*80)
            print("EVALUATING WITH AUTOGLUON")
            print("="*80)
            
            # Load actual test datasets
            test_dataset_ids = ids_test  # From get_data()
            test_datasets = load_test_datasets(test_dataset_ids)
            
            # Results storage
            for k in list(warm_starters.keys()):
                result_to_save[k]['test_time_ag'] = 0.
                result_to_save[k]['ag_accs'] = []
                result_to_save[k]['ag_best_accs'] = []
                result_to_save[k]['ag_ranks'] = []
                result_to_save[k]['ag_top1_hits'] = []
            
            # Evaluate each test dataset
            for d_idx, dataset_id in enumerate(tqdm(test_dataset_ids, desc="Test Datasets")):
                if dataset_id not in test_datasets:
                    print(f"\nSkipping D_{dataset_id} (not loaded)")
                    continue
                
                print(f"\n{'='*80}")
                print(f"Dataset D_{dataset_id} ({d_idx+1}/{len(test_dataset_ids)})")
                print(f"{'='*80}")
                
                X_test_full = test_datasets[dataset_id]['X']
                y_test_full = test_datasets[dataset_id]['y']
                
                # Get dataset features for this test dataset
                ftest = FtestNorm[d_idx, :]
                
                # Evaluate each warm starter
                for k in tqdm(list(warm_starters.keys()), desc="Warm Starters", leave=False):
                    print(f"\n  Method: {k}")
                    
                    start_time = time.time()
                    
                    # Get recommendations from warm starter
                    ix_recommended = warm_starter[k].recommend(ftest)
                    
                    # Evaluate top bo_n_init recommendations with AutoGluon
                    print(f"  Evaluating top {args.bo_n_init} recommendations:")
                    accs = []
                    
                    for i, ix in enumerate(ix_recommended[:args.bo_n_init]):
                        config = PIPELINE_CONFIGS[ix]
                        print(f"    Rank {i+1}: {config['name']:30}", end=" ")
                        
                        acc = evaluate_single_pipeline_autogluon(X_test_full, y_test_full, config)
                        accs.append(acc)
                    ...
                    # Evaluate baseline
                    baseline_acc = evaluate_single_pipeline_autogluon(
                        X_test_full, y_test_full, PIPELINE_CONFIGS[0]
                    )
                    
                    # Evaluate all pipelines to get ground truth ranking
                    print(f"  Evaluating all pipelines for ground truth...")
                    all_accs = evaluate_all_pipelines_on_dataset(X_test_full, y_test_full)
                    
                    # Compute rankings
                    valid_mask = ~np.isnan(all_accs)
                    from scipy.stats import rankdata
                    rankings = np.full(len(all_accs), np.nan)
                    if valid_mask.sum() > 0:
                        rankings[valid_mask] = rankdata(-all_accs[valid_mask], method='average')
                    
                    # Get rank of first recommendation
                    first_rec_idx = ix_recommended[0]
                    first_rec_rank = rankings[first_rec_idx]
                    
                    # Find best pipeline
                    best_idx = np.nanargmax(all_accs)
                    top1_hit = (first_rec_idx == best_idx)
                    
                    # Store results
                    result_to_save[k]['ag_accs'].append(accs)
                    result_to_save[k]['ag_best_accs'].append(all_accs[best_idx])
                    result_to_save[k]['ag_ranks'].append(first_rec_rank)
                    result_to_save[k]['ag_top1_hits'].append(top1_hit)
                    
                    end_time = time.time()
                    result_to_save[k]['test_time_ag'] += (end_time - start_time)
                    
                    # Print summary
                    print(f"\n  Results for {k}:")
                    print(f"    Baseline:        {baseline_acc:.2f}% (Rank {rankings[0]:.1f})")
                    print(f"    Best:            {all_accs[best_idx]:.2f}% (Rank {rankings[best_idx]:.1f})")
                    print(f"    1st Recommended: {all_accs[first_rec_idx]:.2f}% (Rank {first_rec_rank:.1f})")
                    print(f"    Top-1 Hit:       {'✓' if top1_hit else '✗'}")
            ...
            for k in warm_starters.keys():
                avg_rank = np.mean(result_to_save[k]['ag_ranks'])
                top1_rate = np.mean(result_to_save[k]['ag_top1_hits']) * 100
                
                print(f"{k:15} | Avg Rank: {avg_rank:.2f} | Top-1 Rate: {top1_rate:.1f}%")
        # test_accs = {}
